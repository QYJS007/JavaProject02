...
var columns = [[
	...
	{field:'url', title:'目标链接', halign:'center'},
	{field:'urlRegex', title:'链接正则', halign:'center'},
	{field:'nameRegex', title:'名称正则', halign:'center'},
	{field:'nextLayer', title:'作为下一层链接', halign:'center'},
	{field:'nextPage', title:'下一页链接', halign:'center'},
	{field:'encoding', title:'编码', halign:'center',formatter:function(value,row,index){
			return encodingNameValueObj[value];
		}},
	{field:'intervalTime', title:'运行间隔', halign:'center'}

]];

//增删改查的路由
var queryUrl = '/sssweb2/HttpDownTempletBackController/search.do';
var saveUrl = '/sssweb2/HttpDownTempletBackController/save.do';
var deleteUrl = '/sssweb2/HttpDownTempletBackController/delete.do';
...
$(function(){
	...
	//初始化下拉列表
	dictSelect($('#e_encoding'),'t_http_down_templet','encoding',encodingNameValueObj);
	...
	$('#modify').click(function(){
		...
		$('#e_name').val(row.name);
		$('#e_url').val(row.url);
		$('#e_urlRegex').val(row.urlRegex);
		$('#e_nameRegex').val(row.nameRegex);
		$('#e_nextLayer').val(row.nextLayer);
		$('#e_nextPage').val(row.nextPage);
		$('#e_encoding').val(row.encoding);
		$('#e_intervalTime').val(row.intervalTime);
