package work.protec.bus365.codegenerate.payservermngr;
...
public class ServiceMain {
	...
	public String getModelName(String tableName, String tablePrefix){
		...
		if(tableName.equals("businessbaseconfig")){
			...
			modelName = "SellNum";
		}else if(tableName.equals("transfertarget")){
			modelName = "TransferTarget";
		}else{
			//去掉表前缀,按_分割,首字母大写
	...
	public String getPlayJPAConfigName(String dbName){
		...
		}else if(dbName.equals("data") || dbName.equals("ps_pub")){
			configName = "null";
		}else{
...
//		param.autoCompleteList.add("vehicletypeid|vttypename|vehicletype|id|vttypename|id|车型名称");
	...
	@SuppressWarnings("unused")
	private ServiceParam _transfertarget(TableInfo tableInfo){
		ServiceParam param = new ServiceParam();
		//模型的中文描述
		param.modelChineseName = "转账收款方信息";
		//模型字段的中文描述(不指定默认为数据库的字段描述)
//		param.fieldChineseNameMap.put("orgcode", "机构代码");
//		param.fieldChineseNameMap.put("orgname", "机构名称");
		//查询条件
		param.searchParamList.add("businessno");
		param.searchParamList.add("buinessname");
		//查询时不显示的列
//		param.noShowList.add("usercode");
//		param.noShowList.add("username");
//		param.noShowList.add("updatetime");
//		param.noShowList.add("createtime");
		...
		param.noUpdateShowList.add("createtime");
		//修改时不能为空的列
		param.mustFieldList.add("businessno");
		param.mustFieldList.add("buinessname");
		param.mustFieldList.add("targetaccount");
		param.mustFieldList.add("checkurl");
		//修改时显示为 textarea 的列
		param.textAreaList.add("checkurl");
		//数据库不修改的列
		param.noUpdateList.add("createtime");
