...
public class IOUtils {
	...
	/**
	 * 读取文件返回list(可读取word,excel,文本文件),excel列分隔符为◆
	 * @author likaihao
	 * @date 2015年12月17日 上午10:05:50
	 * @param file
	 * @param encoding
	 * @return
	 */
	public static List<String> readFileReturnList(File file,String encoding){
		try {
			if(!file.exists()){
			...
			if(houZhui.equals("doc") || houZhui.equals("docx")){
				//word
				list = OfficeUtils.readWordReturnList(file);
			}else if(houZhui.equals("xls") || houZhui.equals("xlsx")){
				//excel
				list = OfficeUtils.readExcelReturnList(file,"yyyy-MM-dd","◆");
			}else{
	...
	public static List<File> getFileListByPattern(String path,String[] pathPatternArr,String[] noPathPatternArr,String[] fileNamePatternArr,String[] noFileNamePatternArr){
		...
		if(pathPatternArr!=null){
			for(int i=0;i<pathPatternArr.length;i++){
				if(!pathPatternArr[i].startsWith("/")){
					pathPatternArr[i] = "/"+pathPatternArr[i];
				}
			}
		}
		if(noPathPatternArr!=null){
			for(int i=0;i<noPathPatternArr.length;i++){
				if(!noPathPatternArr[i].startsWith("/")){
					noPathPatternArr[i] = "/"+noPathPatternArr[i];
				}
			}
		}
		...
		//按路径正则获得文件
		List<File> fileList = new ArrayList<File>();
		getFileListByPattern2(file,pathPatternArr,noPathPatternArr,fileList,file.getAbsolutePath(),-1);
	 ...
	 * 遍历获得指定路径规则的文件列表
	 * @author likaihao
	 * @date 2016年7月17日 下午3:57:26
	 * @param file
	 * @param pathPatternArr
	 * @param noPathPatternArr
	 * @param fileList
	 * @param basePath
	 * @param index
	 */
  			...
  			if(pathPatternArr!=null && pathPatternArr.length>0){
  				...
  				//遍历每个匹配规则
  	  			for(int i=0;i<pathPatternArr.length;i++){
  	  				...
  	  				//按/分割,第几层就对应前几个/的内容
  	  				pathPattern = pathPattern.substring(1);//去掉前边的/
  	  				String[] subPathPatternArr = pathPattern.replace("[^/]", "@@@").split("/",0);
  	  				String re = "";
  	  				for(int j=0;j<=index&&j<subPathPatternArr.length;j++){
  	  				...
  	  				if(subPath.matches(re)){
  	  					//如果存在.*,则不验证后续的文件夹
	  	  				if(subPathPatternArr[index].equals(".*")){
		  	  				pathPatternArr = null;
		  	  			}
	  	  				//匹配规则,跳出循环继续向下执行代码
  	  					break;
	...
	public static void main(String[] args) {
		String path = SystemConf.getProjectClassPath()+"model";
		
		String[] pathPatternArr = new String[]{};
		String[] noPathPatternArr = new String[]{};
		String[] fileNamePatternArr = new String[]{".*\\.xml"};
		String[] noFileNamePatternArr = new String[]{};
		List<File> list = getFileListByPattern(path,pathPatternArr,noPathPatternArr,fileNamePatternArr,noFileNamePatternArr);
		for(File f : list){
