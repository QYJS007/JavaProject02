...
先思考这样一个问题，现在我们有一堆	size大小相同	而颜色不同的数据，
...
我们有如下解决方案，使用Map集合来统计，key	值作为颜色名称，value	代表衣服数量，如下：
 ...
 * 	一种是HashMap，
 * 	一种EnumMap，
 * 虽然都统计出了正确的结果，但是EnumMap作为枚举的专属的集合，
 * 	我们没有理由再去使用HashMap，毕竟EnumMap要求其Key必须为Enum类型，
 * 	因而使用Color枚举实例作为key是最恰当不过了，也避免了获取name的步骤，更重要的是EnumMap效率更高
 * ，因为其内部是通过数组实现的（稍后分析），
 * 
 * 注意EnumMap的key值不能为null，虽说是枚举专属集合，但其操作与一般的Map差不多，概括性来说EnumMap是专门为枚举类型量身定做的Map实现，
 * 
 * 虽然使用其它的Map（如HashMap）也能完成相同的功能，但是使用EnumMap会更加高效，它只能接收同一枚举类型的实例作为键值且不能为null，+
 * 
...
//EnumMap(Map<K,? extends V> m)       
 //与HashMap不同，它需要传递一个类型信息，即Class对象，通过这个参数EnumMap就可以根据类型信息初始化其内部数据结构，另外两只是初始化时传入一个Map集合，
...
/*
 * 至于EnumMap的方法，跟普通的map几乎没有区别，
 * 
 * 注意与HashMap的主要不同在于构造方法需要		传递类型参数和EnumMap保证Key顺序与枚举中的顺序一致，但请记住Key不能为null。
*/
//EnumMap实现原理剖析
...
/** 
 * EnumMap继承了AbstractMap类，
 *  因此EnumMap具备一般map的使用方法，
 *  	keyType表示类型信息，
 *  	keyUniverse表示键数组，存储的是所有可能的枚举值，
 *  	vals数组表示键对应的值，
 *  	size表示键值对个数。
 *  
 *  	在构造函数中通过keyUniverse = getKeyUniverse(keyType);
 *  
 *  	初始化了keyUniverse数组的值，内部存储的是所有可能的枚举值，接着初始化了存在Value值得数组vals，
 *  	
 *  	其大小与枚举实例的个数相同，getKeyUniverse方法实现如下
*/
//返回枚举数组
...
/*
 * 从方法的返回值来看，返回类型是枚举数组，
 * 事实也是如此，最终返回值正是枚举类型的values方法的返回值，前面我们分析过values方法返回所有可能的枚举值，
 * 
 * 因此keyUniverse数组存储就是枚举类型的所有可能的枚举值。接着看put方法的实现
 * 
...
/*//  判断value直接通过遍历数组实现，而判断key就更简单了，判断key是否有效和对应vals[]中是否存在该值。
 * 
 * 
 * ok~，这就是EnumMap的主要实现原理，即内部有两个数组，长度相同，一个表示所有可能的键(枚举值)，一个表示对应的值，
 * 
 * 
 * 不允许keynull，但允许value为null，键都有一个对应的索引，根据索引直接访问和操作其键数组和值数组，由于操作都是数组，因此效率很高。
*/
